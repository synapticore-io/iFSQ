...
class GFQ(Module):
    def __init__(
        ...
        *,
        ...
        self.codebook_size = codebook_size = default(codebook_size, lambda: 2**dim)
        ...
        self.token_factorization = factorized_bits is not None
    ...
    def forward(
        ...
        if self.token_factorization:
            ...
            for i, factorized_bit in enumerate(self.factorized_bits):
                ...
                indices = reduce(
                    (quantized[..., begin:end] > 0).int() * mask.int(),
                    "b n c d -> b n c",
                    "sum",
                )
...
class RunningLFQ(GFQ):
    def __init__(self, eps=1e-6, momentum=0.1, **kwargs):
        super(RunningLFQ, self).__init__(**kwargs)
    ...
    def __repr__(self):
        # Getting the default string representation
        repr_str = super().__repr__()

        # Adding buffer information
        buffer_info = "\nBuffers:\n"
        for name, buffer in self.named_buffers():
            buffer_info += f"  {name}: {buffer.shape}\n"

        return repr_str + buffer_info
