...
from ..modules.fsq import GroupFSQ, RunningFSQ
...
@ModelRegistry.register("ImageFSQVAE")
class ImageFSQVAEModel(ImageBaseAE):
    @register_to_config
    def __init__(
        self,
        hidden_size: int = 128,
        ...
        factorized_bits: Optional[List[int]] = None,
        ...
        self.quantize = RunningFSQ(
            levels=levels,
            dim=embed_dim,
            num_codebooks=1,
            factorized_bits=factorized_bits,
            do_simple_bound=do_simple_bound,
            act_fun=act_fun,
        )
    ...
    def encode(self, x):
        ...
        if self.use_quant_layer:
            h = self.quant_conv(h)
        out, indices = self.quantize(h)
        return EncoderOutput(sample=out, indices=indices)
    ...
    def forward(self, input, sample_posterior=True):
        z = self.encode(input).sample
        dec = self.decode(z).sample
        return ForwardOutput(sample=dec)
