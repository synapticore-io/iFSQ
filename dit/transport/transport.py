...
class Transport:
    ...
    def sample_cosine(size=1):
        ...
        return th.tensor(
            np.random.choice(x, size=size, p=weights / weights.sum()), dtype=th.float32
        )
    ...
    def sample(
        ...
        if not self.use_lognorm:
            ...
            if timestep_sampling == "uniform":
                if self.partitial_train is not None and th.rand(1) < self.partial_ratio:
                    t = (
                        th.rand((x1.shape[0],))
                        * (self.partitial_train[1] - self.partitial_train[0])
                ...
                t = (
                    self.sample_beta(beta_alpha, beta_beta, size=x1.shape[0])
                    * (t1 - t0)
                    + t0
                )
                ...
                raise NotImplementedError(
                    f"Not implemented timestep_sampling ({timestep_sampling})"
                )
        else:
            assert timestep_sampling == "lognorm"
            ...
            if not self.shift_lg:
                if self.partitial_train is not None and th.rand(1) < self.partial_ratio:
                    ...
                    t = (
                        self.sample_logit_normal(0, 1, size=x1.shape[0]) * (t1 - t0)
                        + t0
                    )
            else:
                assert (
                ...
                t = (
                    self.sample_logit_normal(shifted_mu, 1, size=x1.shape[0])
                    * (t1 - t0)
                    + t0
                )
        ...
        if sp_timesteps is not None:
            ...
            t = (
                th.rand((x1.shape[0],)) * (sp_timesteps[1] - sp_timesteps[0])
                + sp_timesteps[0]
            )
        t = t.to(x1)
        return t, x0, x1
    ...
    def training_losses(
        ...
        if self.model_type == ModelType.VELOCITY:
            terms["loss"] = (
                mean_flat(((model_output - ut) ** 2))
                if l2_loss
                else mean_flat((model_output - ut).abs())
            )
            if self.use_cosine_loss:
                terms["cos_loss"] = mean_flat(
                    1 - th.nn.functional.cosine_similarity(model_output, ut, dim=1)
                )
            ...
            if self.model_type == ModelType.NOISE:
                terms["loss"] = (
                    mean_flat(weight * ((model_output - x0) ** 2))
                    if l2_loss
                    else mean_flat(weight * ((model_output - x0).abs()))
                )
            else:
                terms["loss"] = (
                    mean_flat(weight * ((model_output * sigma_t + x0) ** 2))
                    if l2_loss
                    else mean_flat(weight * ((model_output * sigma_t + x0).abs()))
                )
