...
@torch.no_grad()
def load_encoders(
    enc_type, device, resolution=256, downsample_ratio=16, dtype=torch.bfloat16
):
    ...
    for enc_name in enc_names:
        ...
        if resolution != 256:
            if encoder_type != "dinov2":
                raise NotImplementedError(
                    "Currently, we only support !=256x256 experiments with DINOv2 encoders."
                )
        ...
        if encoder_type == "mocov3":
            ...
            del encoder.head
            patch_resolution = 16 * (16 // downsample_ratio) * (resolution // 256)
        ...
        encoders.append(encoder.to(dtype=dtype))
