...
class MAR_VAE:
    def __init__(self, ckpt_path, img_size=256, horizon_flip=0.5, fp16=True):
        self.embed_dim = 16
        self.ckpt_path = (
            ckpt_path  # <-- MAR VAE checkpoint, download it from its official repo
        )
...
if __name__ == "__main__":
    ...
    print(vae.model)
    print(
        f"MAR_VAE Parameters: {sum(p.numel() for p in vae.model.parameters()) / 1e6:.2f}M"
    )
    print(
        f"MAR_VAE Encoder Parameters: {sum(p.numel() for p in vae.model.encoder.parameters()) / 1e6:.2f}M"
    )
    print(
        f"MAR_VAE Decoder Parameters: {sum(p.numel() for p in vae.model.decoder.parameters()) / 1e6:.2f}M"
    )
