...
class DiT(nn.Module):
    ...
    def shuffle_pos_embed(self, x):
        ...
        shuffle_mask = (
            torch.rand(B, N, device=x.device) < self.shuffle_ratio
        )  # B x N，True 表示要打乱
        ...
        for i in range(B):
            ...
            shuffled_tokens = indices[i, mask][
                torch.randperm(mask.sum())
            ]  # 只打乱 mask 选中的部分
            shuffle_indices[i, mask] = shuffled_tokens  # 只替换打乱的部分

        # === Step 3: 使用 `gather` 进行并行 shuffle ===
        x_shuffled = torch.gather(
            x, dim=1, index=shuffle_indices.unsqueeze(-1).expand(-1, -1, D)
        )
    ...
    def forward(self, x, t=None, y=None, **kwargs):
        ...
        use_checkpoint: boolean to toggle checkpointing
        """
        x, _, c, layer_logits = self.forward_feature(x, t, y, **kwargs)
    ...
    def forward_with_cfg(
        self, x, t, y, cfg_scale, cfg_interval=None, cfg_interval_start=None
    ):
        """
...
DiT_models = {
    ...
    "DiT-XL/1": DiT_XL_1,
    "DiT-XL/2": DiT_XL_2,
    "DiT-1p0B/1": DiT_1p0B_1,
    "DiT-1p0B/2": DiT_1p0B_2,
    "DiT-1p6B/1": DiT_1p6B_1,
    "DiT-1p6B/2": DiT_1p6B_2,
}
